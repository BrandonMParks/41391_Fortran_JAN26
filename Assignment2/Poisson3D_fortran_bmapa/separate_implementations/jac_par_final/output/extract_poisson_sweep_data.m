...
if exist('outFile', 'var') == 1 && ~isempty(outFile)
    ...
    listing = dir(fullfile(scriptDir, 'Output_*.out'));
    if isempty(listing)
...
if isempty(N_tok) || isempty(itermax_tok) || isempty(tol_tok) || isempty(alg_tok) || isempty(omp_dyn_tok)
...
pairTokens = regexp(txt, [ ...
    ...
    'GS:\s*total parallel update wall time \(s\)\s*=\s*([0-9Ee+\-\.]+)'], ...
    'tokens');

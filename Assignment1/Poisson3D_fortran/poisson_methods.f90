...
! ------------------------------------------------------------------
  ...
  !
    ...
    ! Improvement (1): avoid fork/join each iteration by using a single
    ! OpenMP parallel region around the iteration loop.
    !$omp parallel default(shared) private(i, j, k, u_new_val, diff, iter)
    !$omp single
    ...
    do iter = 1, itermax
      ...
      !$omp do collapse(3) schedule(static) reduction(max:diffmax)
      do k = 2, n - 1
        do j = 2, n - 1
          do i = 2, n - 1
            u_new_val = ( &

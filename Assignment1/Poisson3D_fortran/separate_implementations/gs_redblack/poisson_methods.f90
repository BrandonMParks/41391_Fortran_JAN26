...
! ------------------------------------------------------------------
  ...
  !
    ...
    print '(a)', 'Entering gauss_seidel; first parallel region beginning.'
    call flush(6)

    !$omp parallel default(shared) private(i, j, k, u_new_val, iter)
    !$omp master
    threads_used_local = omp_get_num_threads()
    print '(a,i0)', 'OpenMP: gauss_seidel threads used = ', threads_used_local
    !$omp end master

    do iter = 1, itermax
      ! update one color in parallel, then the other
      !$omp master
      t0_par = omp_get_wtime()
      !$omp end master

      ! Color 1 sweep: points where (i+j+k) is even.
      !$omp do collapse(2) schedule(static)
      do k = 2, n - 1
        do j = 2, n - 1
          do i = 2 + mod(j + k, 2), n - 1, 2
            u_new_val = ( &
              u(i - 1, j, k) + u(i + 1, j, k) + &
              u(i, j - 1, k) + u(i, j + 1, k) + &
              u(i, j, k - 1) + u(i, j, k + 1) + &
      ...
      ! Color 2 sweep: points where (i+j+k) is odd.
      !$omp do collapse(2) schedule(static)
      do k = 2, n - 1
        do j = 2, n - 1
          do i = 2 + mod(j + k + 1, 2), n - 1, 2
            u_new_val = ( &
              u(i - 1, j, k) + u(i + 1, j, k) + &
              u(i, j - 1, k) + u(i, j + 1, k) + &
              u(i, j, k - 1) + u(i, j, k + 1) + &
      ...
      ! !$omp master
      ! if (mod(iter, 100) == 0) then
      !   print '(a,i0)', 'gauss_seidel: iter=', iter
      ! end if
      ! !$omp end master
    end do
    !$omp end parallel

    print '(a)', 'Exiting gauss_seidel; last OpenMP region completed'
    call flush(6)

    write(*,'(a,es23.16)') 'gauss_seidel: total parallel update wall time (s) = ', t_par_total

    if (present(iters_done)) iters_done = iter
    if (present(diffmax_final)) diffmax_final = -1._dp
    if (present(t_total)) t_total = omp_get_wtime() - t_total_start
